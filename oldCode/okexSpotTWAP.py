        ...
        if 'data' in message:
            for data in message['data']:
                if data['state'] == '2':
                    ...
                    execute(db_name=f"TWAP_OKEX_{self.coin.replace('-', '')}_SPOT",
                            exchange='OKEX',
                            market=self.market,
                            ...
                            time=datetime.strptime(data['timestamp'], "%Y-%m-%dT%H:%M:%S.%fZ").strftime(
                                '%Y-%m-%d %H:%M:%S'),
                            price=float(data['filled_notional']) / float(data['filled_size']),
                            side=self.side,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Okex Spot')
