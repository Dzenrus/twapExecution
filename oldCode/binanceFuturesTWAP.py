...
class BinanceFuturesTWAP(BinanceSimpleTWAP):

    def __init__(self, exchange, market, coin, qty, side, execution_minutes, execution_freq_per_minute, cont='True',
                 leverage=1):
        super().__init__(exchange, market, coin, qty, side, execution_minutes, execution_freq_per_minute, cont)
    ...
    def run(self):
        ...
        self.binance.rest_client.post_change_initial_margin(self.coin, self.leverage)
        self.binance.rest_client.post_position_mode('false')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Binance Futures')
    parser.add_argument('--args', nargs='+', help='')

    args = parser.parse_args()

    # print(args.args)
    twap = BinanceFuturesTWAP(*args.args)
    try:
        twap.run()
    except:
        twap.binance.close()
